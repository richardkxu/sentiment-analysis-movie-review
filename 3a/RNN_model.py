...
class StatefulLSTM(nn.Module):
    ...
    def forward(self,x):
        ...
        if self.h is None:
            ...
            self.c = Variable(torch.zeros(state_size)).cuda()  # initial hidden state
            self.h = Variable(torch.zeros(state_size)).cuda()  # initial cell state
        # next hidden state, next cell state
...
# It has been shown to be more effective to use the same dropout
# mask for an entire sequence as opposed to a different dropout mask each time
...
class RNN_language_model(nn.Module):
    ...
    def forward(self, x, train=True):
        ...
        for i in range(no_of_timesteps):
            ...
            outputs.append(h)  # a probability distribution over the entire vocabulary
